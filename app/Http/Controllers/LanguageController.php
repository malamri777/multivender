...
use Modules\Translations\Entities\AppTranslation;
...
class LanguageController extends Controller
{
    ...
    public function exportLangFile($id)
    {
        $language = Language::findOrFail($id);

        if ($language) {
            $contents = file_get_contents(module_path('translations') . "/Database/LanguageExport/app_{$language->app_lang_code}.arb");
            foreach (json_decode($contents) as $key => $value) {
                info($key);
                info($value);
