    ...
    <form class="" id="sort_products" action="" method="GET">
        <div class="card-header row gutters-5">
            ...
            <div class="col-md-2 ml-auto">
                <select class="form-control form-control-sm aiz-selectpicker mb-2 mb-md-0" id="supplier_id" name="supplier_id"
                    onchange="sort_products()">
                    <option value="">{{ translate('All Suppliers') }}</option>
                    @foreach (App\Models\Supplier::where('verification_status', 1)->get() as $key => $supplier)
            ...
            <div class="col-md-2 ml-auto">
                <select class="form-control form-control-sm aiz-selectpicker mb-2 mb-md-0" id="user_id" name="user_id"
                    ...
                    @foreach (App\Models\User::where('user_type', '=', 'admin')->orWhere('user_type', '=',
                    'seller')->get() as $key => $seller)
                    <option value="{{ $seller->id }}" @if ($seller->id == $seller_id) selected @endif>{{ $seller->name
                        }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <table class="table aiz-table mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <label class="aiz-switch aiz-switch-success mb-0">
                                <input onchange="update_featured(this)" value="{{ $product->id }}" type="checkbox" <?php
                        ...
                        <td class="text-right">
                            ...
                            <a class="btn btn-soft-primary btn-icon btn-circle btn-sm"
                                href="{{route('admin.products.seller.edit', ['id'=>$product->id, 'lang'=>config('myenv.DEFAULT_LANGUAGE')] )}}"
                                title="{{ translate('Edit') }}">
                                <i class="las la-edit"></i>
                            </a>
                            @else
                            <a class="btn btn-soft-primary btn-icon btn-circle btn-sm"
                                href="{{route('admin.products.admin.edit', ['id'=>$product->id, 'lang'=>config('myenv.DEFAULT_LANGUAGE')] )}}"
                                title="{{ translate('Edit') }}">
                                <i class="las la-edit"></i>
                            </a>
                            @endif
                            <a class="btn btn-soft-warning btn-icon btn-circle btn-sm"
                                href="{{route('admin.products.duplicate', ['id'=>$product->id, 'type'=>$type]  )}}"
                                title="{{ translate('Duplicate') }}">
                                <i class="las la-copy"></i>
                            </a>
                            <a href="#" class="btn btn-soft-danger btn-icon btn-circle btn-sm confirm-delete"
                                data-href="{{route('admin.products.destroy', $product->id)}}"
