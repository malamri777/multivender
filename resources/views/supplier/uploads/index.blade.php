...
<div class="aiz-titlebar text-left mt-2 mb-3">
    <div class="row align-items-center">
        ...
        <div class="col-md-6 text-md-right">
            ...
            @endphp
            <a href="{{ route('supplier.uploaded-files.index', ['folder_id' => $folder->parent?->id]) }}"
                class="btn btn-link text-reset">
                <i class="las la-angle-left"></i>
                <span>{{translate('Back to uploaded files')}}</span>
            </a>
            <a href="{{ route('supplier.uploaded-files.create', ['folder_id' => $folder->id]) }}"
                class="btn btn-primary">
                <span>{{translate('Upload New File')}}</span>
            </a>
            @else
            <a href="{{ route('supplier.uploaded-files.create') }}" class="btn btn-primary">
                <span>{{translate('Upload New File')}}</span>
            </a>
            @endif
...
<div class="card">
        ...
        <div class="card-header row gutters-5">
            ...
            <div class="col-md-3 ml-auto mr-0">
                <select class="form-control form-control-xs aiz-selectpicker" name="sort" onchange="sort_uploads()">
                    <option value="all" @if($sort_by=='all' or $sort_by==null) selected="" @endif>{{ translate('All') }}
                    </option>
                    <option value="newest" @if($sort_by=='newest' ) selected="" @endif>{{ translate('Sort by newest') }}
                    </option>
                    <option value="oldest" @if($sort_by=='oldest' ) selected="" @endif>{{ translate('Sort by oldest') }}
                    </option>
                    <option value="smallest" @if($sort_by=='smallest' ) selected="" @endif>{{ translate('Sort by
                        smallest') }}</option>
                    <option value="largest" @if($sort_by=='largest' ) selected="" @endif>{{ translate('Sort by largest')
                        }}</option>
                </select>
            </div>
            <div class="col-md-3">
                        ...
                        <div class="aiz-file-box">
                            <div class="dropdown-file">
                                ...
                                <div class="dropdown-menu dropdown-menu-right">
                                    <a href="javascript:void(0)" class="dropdown-item confirm-alert"
                                        data-href="{{ route('admin.uploaded-files.destroy', $file->id ) }}"
                            ...
                            <a href="{{ route('admin.uploaded-files.index', ['folder_id' => $file->id]) }}">
                                <div class="card card-file aiz-uploader-select c-default" title="{{ $file->folder_name }}">
                                    ...
                                    <div class="card-body">
                                        <h6 class="d-flex">
                                            <span class="text-truncate title" style="color: black">{{ $file->folder_name
                                                }}</span>
                                        </h6>
        ...
        <div class="modal-dialog modal-sm modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body text-center">
                    <input type="text" class="form-control mb-4" name="folder-name" id="folder-name"
                        placeholder="{{ translate('Enter folder name') }}">
                    <button type="button" class="btn btn-link mt-2" data-dismiss="modal">{{ translate('Cancel')
                        }}</button>
    ...
    <script type="text/javascript">
		...
		function detailsInfo(e){
			...
			$.post('{{ route('supplier.uploaded-files.info') }}', {_token: AIZ.data.csrf, id:id}, function(data){
        ...
        $('.create-folder-el').click(() => {
            ...
            $.post('{{ route('supplier.uploaded-files.createFolder', ['folder_id' => Request::get('folder_id') ?? ""]) }}', {_token: AIZ.data.csrf, name:folderNameVal}, function(data){
                // console.log(data);
